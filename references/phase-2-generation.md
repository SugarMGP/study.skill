...
- teaching patterns from `references/teaching-patterns.md` only where they fit the concept; one well-used pattern is enough, and a section may use none when a plain worked explanation is clearer
...
- 教科书压缩感抽检：随机抽 2-3 个小节，执行 `courseware-format.md` GC1-GC5 规则检查。连续 3 个概念块间质元素不足，或靠固定口头禅制造节奏的，打回重写。
...
- Consume the Phase 1 teaching asset ledger before drafting prose. For every Foundation/Core section, select the strongest relevant asset and carry over the useful teaching move: the source's opening problem, explanation order, worked example, intermediate steps, contrast, diagram, or exercise transition. A bibliography entry or inline link alone does not count as using the asset.
- Keep the ledger as an internal working note while generating. Mark each asset `used`, `moved`, or `omitted: {scope reason}`. Do not create a new learner-facing resource file for it.
...
| Diagram or equivalent visual structure | [SHOULD] if structure is complex; else table/examples OK. Must use renderable formats (mermaid/plantuml/graphviz/d2/vega-lite); ASCII art/character drawings do not count as diagrams. See `courseware-format.md` diagram rules. | [MUST] when content involves flow/architecture/hierarchy/contrast/dependency; else table/examples OK. Must use renderable formats; ASCII art/character drawings do not count. | [SHOULD] |
...
| Teaching asset adoption | [MUST] each section uses a concrete researched example, explanation move, figure, or exercise where available | [MUST] the strongest source teaching assets are visibly transformed into the lesson; citations alone do not pass | [SHOULD] |
...
| Worked support -> faded practice -> independent retrieval | [SHOULD] for multi-step novice tasks; skip extra layers for trivial one-step tasks | [MUST] for new multi-step procedural or problem-solving skills; support must decrease meaningfully | [SHOULD] when the enrichment task is assessed |
...
4. **Complete demonstrations**: For every code, SQL, formula, or procedure section, is there a full worked example with input/output/explanation? Compare the task wording, conditions, input, steps, and final result: do they solve the same problem, including qualifiers such as latest/all/each and the direction of conditional events? Check that every formula uses one of the exact paired delimiters from `courseware-format.md`. No keyword-only, snippet-only, malformed-formula, or internally contradictory teaching.
5. **Exercise quality**: Do core-module exercises include at least one apply/analyze/explain question (not all recall-only)? For a new multi-step skill, does a complete demonstration lead into meaningfully faded practice and then an independent check, without turning that sequence into fixed headings or duplicate questions? Are answer/explanation fields inside `study-*` blocks (not visible before submit)?
6. **Source coverage and adoption**: Are all Phase 1 researched important points taught in a section, or deliberately omitted with a scoped reason? For sampled Foundation/Core sections, can the reviewer point to the researched teaching asset that shaped the explanation, example, diagram, or exercise? A source list without visible adoption fails. If the stated skill is primary-source, legal-text, literature, or paper reading, verify that the learner actually reads an embedded excerpt or image in each core analysis page; an agent-written summary does not count as source adoption.
...
13. **No textbook compression or synthetic teacher voice**: For a randomly chosen section from each module, read the first 200 words aloud. Does it sound like a teacher following one problem, or like a dictionary padded with "我们来看看" and progress announcements? Apply `courseware-format.md` GC1-GC5: concepts have breathing room (GC1), core concept clusters have complete demonstrations (GC2), formulas have numeric walkthroughs (GC3), navigation appears only at real transitions (GC4), and teacher voice performs a concrete job rather than meeting a phrase quota (GC5).
