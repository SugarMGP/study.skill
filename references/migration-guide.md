...
- 迁移脚本保留旧文件，不删除 `progress.json` 或 `review-schedule.json`
...
2. 为每门课程创建 `.learning-profile/courses/{slug}/meta.json`。
...
5. 验证新结构后，再考虑删除旧的 `progress.json` 和 `review-schedule.json`。
