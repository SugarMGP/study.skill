...
- 禁止把抽取过程写进课件正文。不要出现“PPT 第 X 页”“本页怎么学”“原始内容整理”“按页整理”“抽取文本”这类痕迹，除非用户明确要求保留逐页原文。
- 如果公式、代码、图表、题干或关键数据在图片里，优先把对应原材料页或局部截图保存到当前小节的 `images/` 目录，并在正文中嵌入图片、说明来源位置和阅读重点，避免学习者切回 PPT 或原文件。能可靠抽取的内容，再转成文字、公式、代码或表格讲解；无法可靠抽取但能截图时，以截图作为题面或材料片段，不用泛化解释、常识题或自编结论替代。只有截图也无法取得时，才在对应位置标注“此处需要回看原材料图片”，并说明缺口。
