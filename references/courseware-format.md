...
- 对于用户提供的资料或其他外部资料，优先保留材料的结构编排、原文片段、原图、原表、原例子、原题和原代码，再在前后补过渡、背景、解释和练习。允许大段照搬原文，不需要考虑版权问题。不要为了“原创”或“衔接”删掉这些教学内容，或者把它们改成提纲式摘要。
...
GC1-GC5 规则聚焦于段落间的信息密度节奏，与现有的实质审查规则（禁止提纲式摘要、关键词清单、讲义化不足）互补。现有规则检查"有没有讲"，GC 规则检查"讲得散不散"——一个通过了所有现有质量门的小节，仍可能因段落间缺乏间质元素而呈压缩感。
...
### GC4：段落内 pacing marker 要求（所有超过 400 字/英文 250 words 的小节 [MUST]）

正文必须包含至少一句显式 pacing marker——告诉学习者现在在什么学习阶段：
- "现在你理解了 X，我们可以开始..."
- "先停下来想一想..."
- "动手试之前先看清..."
- "到这里我们已经完成了..."

### GC5：教学声音密度要求（所有小节 [MUST]）

每个小节中，以下"教师声音标记"至少出现 3 次（中文课件）：
- 直接称呼（你、我们）
- 祈使句（试试、注意、检查一下、回头看）
- 提问句（为什么、如果...会怎样）
- 过渡/导航句（先/再/然后、这一节解决的是）

纯陈述句连续超过 10 句即触发密度不足警告。
...
| 无直接对应 | GC4 pacing marker | 新建规则——现有规则体系没有段落级节奏断点的检查 |
| 无直接对应 | GC5 教学声音密度 | 新建规则——现有规则体系没有教师声音标记密度的检查 |

## 质量自检

以下为快速自查项，不可替代 `phase-2-generation.md` 的 13 项阻塞审查。

注意：`scripts/check-course-depth.py` 的抽取痕迹和模板痕迹扫描词为中文（如"PPT 第"、"本页怎么学"）。英文课程需在 Phase 2 阻塞审查中由 agent 手动检查类似痕迹（如"Page X of PPT"、"original content organized"）。

生成课程文件后，至少检查：

- 正文是否以知识点组织而非按页码堆叠？连续出现 5 个以上 `### 原课件页 N` 或 `### 第 X 页` 标题属于逐页贴图，不合格。
- 所有 `study-*` 块的 `answer` 字段是否为学生可直接对照的具体答案（而非“参考答案应包含…”等占位符）？
