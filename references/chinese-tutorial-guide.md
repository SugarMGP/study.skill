...
课程正文必须先面向学习者写清楚题目，再按需补一个可保存练习块。
这些代码块是给课程播放器读取的，不要在正文里解释它们的实现原理。

优先级：

1. **回忆题 `study-recall`**：刚讲完一个关键概念后使用。
2. **迁移题 `study-transfer`**：核心模块优先使用，要求换场景应用。
3. **费曼解释 `study-feynman`**：要求学习者用自己的话解释概念。
4. **模块检查 `study-checkpoint`**：模块末尾使用一次，引用前面的练习。
...
先不看答案，用自己的话说说一次训练循环里发生了什么。

```study-feynman
id: training-loop-feynman
concept: 训练循环
prompt: 用自己的话解释一次训练循环从输入数据到参数更新的过程
key_points: 前向计算、损失函数、反向传播、参数更新
```
````

写法规则：

- `id` 用小写英文、数字和连字符，整门课内稳定唯一。
- 问题要像正常教材题，不要写成系统字段说明。
- 参考答案只放参考思路，不要写“判定通过/不通过”。
- 不要在课程文件中记录用户是否答对、是否通过、是否掌握。
...
调研阶段（Phase 1）同步收集官方文档、教程中的现成图表。无需顾虑版权问题，只需要注明来源即可。满足以下条件则直接使用：
- 清晰可读，无模糊/破损
- 内容与当前教学点直接相关
...
请你自查自己有没有生成图片功能，或者是内置 imagegen 等类似技能。使用这些能力生成 PNG/SVG，嵌入 `![](path/to/diagram.png)`。
